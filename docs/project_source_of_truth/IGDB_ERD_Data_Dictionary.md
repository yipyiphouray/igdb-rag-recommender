...
| `games` | `summary` | Game Summary | TEXT | N/A | Free text; nullable | Game description. | Direct IGDB field. | Not required by the current unfiltered extraction. |
...
| `games` | `game_status_id` | Game Status ID | INTEGER | N/A | FK to `game_statuses`; nullable | Explicit game availability/status value. | Direct IGDB reference. | Current loaded values include `Early Access`, `Offline`, `Cancelled`, `Rumored`, and `Delisted`; null does not prove release status. |
...
| `games` | `cover_id` | Selected Cover ID | INTEGER | N/A | IGDB cover ID; nullable | Cover selected on the game record. | Renamed from IGDB `cover`. | Current non-null values resolve locally, but no SQLite FK is declared. |
...
| `game_statuses` | `status_name`    | Game Status Name    | TEXT      | N/A  | Controlled vocabulary   | Human-readable explicit game status. | Renamed from IGDB game status field. | Current extract contains `Early Access`, `Offline`, `Cancelled`, `Rumored`, and `Delisted`; release-date lifecycle values are in `release_date_statuses`. |
...
| `release_dates` | `platform_id`            | Platform ID            | INTEGER   | N/A          | Must exist in `platforms.platform_id`; nullable                        | Platform associated with the release date.           | Foreign key from IGDB release date record.                    | Useful for platform-specific release analysis; invalid optional source platform references are loaded as null. |
...
| `external_games` | `platform_id`             | Platform ID             | INTEGER   | N/A  | Must exist in `platforms.platform_id`; nullable                         | Platform associated with the external record.          | Foreign key from IGDB external game record.          | Useful for external store/platform matching; invalid optional source platform references are loaded as null. |
...
| `multiplayer_modes` | `platform_id`         | Platform ID         | INTEGER   | N/A     | Must exist in `platforms.platform_id`; nullable | Platform where the multiplayer support applies.          | Foreign key from IGDB multiplayer mode record.      | Multiplayer support may vary by platform; invalid optional source platform references are loaded as null. |
